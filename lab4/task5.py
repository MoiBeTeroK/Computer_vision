...
def task1(image_path: str, kernel_size=5, sigma=5):
...
def task4(suppressed, low_ratio=0.05, high_ratio=0.15):
    highThreshold = suppressed.max() * high_ratio
...
def canny_experiment(image_path, kernel_size=5, sigma=2, low_ratio=0.05, high_ratio=0.15):
    blurred = task1(image_path, kernel_size, sigma)
    Gx, Gy, magnitude, angle = task2(blurred)
    suppressed = task3(magnitude, angle)
    edges = task4(suppressed, low_ratio, high_ratio)
    return edges

edges = canny_experiment(
    "C:/imageLab/akula.jpg",
    kernel_size=3,  # размер ядра Гаусса
    sigma=3,        # sigma для Гаусса
    low_ratio=0.05, # низкий порог градиента
    high_ratio=0.1 # высокий порог градиента
)
